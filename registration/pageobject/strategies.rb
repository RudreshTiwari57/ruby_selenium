...
class Locators
    ...
    def selected(path)
        if path.downcase.include?("tag")
            element = @driver.find_element( tag_name: @a.inireader(path)).selected?
        elsif path.downcase.include?("class")
            element = @driver.find_element(class: @a.inireader(path)).selected?
        elsif path.downcase.include?("id")
            element = @driver.find_element(id: @a.inireader(path)).selected?
        elsif path.downcase.include?("name")
            element = @driver.find_element(name: @a.inireader(path)).selected?
        elsif path.downcase.include?("css")
            element = @driver.find_element(css: @a.inireader(path)).selected?
        elsif path.downcase.include?("link_text")
            element = @driver.find_element(link_text: @a.inireader(path)).selected?
        elsif path.downcase.include?("partial_link_text")
            element = @driver.find_element(partial_link_text: @a.inireader(path)).selected?
        elsif path.downcase.include?("xpath")
            element = @driver.find_element(xpath: @a.inireader(path)).selected?
        end
        return element
    end

    def click_radio(path)
        if path.downcase.include?("tag")
            element = @driver.find_element( tag_name: @a.inireader(path))
        elsif path.downcase.include?("class")
            element = @driver.find_element(class: @a.inireader(path))
